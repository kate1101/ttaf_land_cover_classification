                             ...
                             const int idx_max, map<pair<uint,uint>, pair<bool, uint>>& isLoaded)
{
    ...
    long long int pointNum = header.GetPointRecordsCount();
    ...
    try
    {
        ...
        while (reader.ReadNextPoint() && idx_actual!=idx_max)
        {
            ...
            //map<pair<unsigned int, unsigned int>, pair<bool, unsigned int>>::iterator it = isLoaded.find(make_pair(x, y));
            //if(it != isLoaded.end()) {
...
bool ReadWriteData::writeClassDefault(const string& fileNameIn, const string& fileNameOut, vector<LasPoint>& points)
{
    try
    {
        ifstream ifs(fileNameIn, ios::in | ios::binary);
        ofstream ofs(fileNameOut, ios::out | ios::binary);
        ReaderFactory f;
        Reader reader = f.CreateWithStream(ifs);
        Header const& header = reader.GetHeader();
        header.Compressed();
        Writer writer(ofs, header);

        for (const auto& fixPoint : points)
        {
            reader.ReadNextPoint();
            Point const &tmpPoint = reader.GetPoint();
            Point point = tmpPoint;
            const uint16_t cls= (uint16_t)fixPoint.getClsNum();
            point.SetIntensity(cls);
...
bool ReadWriteData::writeClassAlg(const string& fileNameIn, const string& fileNameOut, vector<LasPoint> &points, bool alg1)
{
    try
    {
        ifstream ifs(fileNameIn, ios::in | ios::binary);
        ...
        for (const auto& fixPoint : points)
        {
            ...
            if (alg1)
            {
                point.SetIntensity((uint16_t)fixPoint.getAlg1());
            }
            else
            {
                point.SetIntensity((uint16_t)fixPoint.getAlg2());
            }
            writer.WritePoint(point);
        }
        retVal = true;
