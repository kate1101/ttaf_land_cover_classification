...
void  CalcParams::calcIDMs(vector<LasPoint>& points, LandCoverParams& param)
{
    ...
    for (const auto& point : points)// idx = 0; idx < points.size(); ++idx)
    {
...
void  CalcParams::calcSkMs(vector<LasPoint>& points, LandCoverParams& param) //ferdeség
{
...
void  CalcParams::calcKrMs(vector<LasPoint>& points, LandCoverParams& param) //csúcsosság
{
...
void  CalcParams::calcSkMcs(vector<LasPoint>& points, LandCoverParams& param)
{
    ...
    long long int pow2sum = 0, pow3sum = 0;
    ...
    for (const auto& point : points)
    {
        if(point.getRetNum() != point.getAllRet() && (point.getZ() -  param.getTM()) > 0.1 * param.getVHM())
        {
            pointNum++;
            zsum += point.getZ();
        }
    ...
    if (pointNum > 1)
    {
        ...
        for (const auto &point : points) {
            pow3sum += pow(point.getZ() - zsum, 3);
            pow2sum += pow(point.getZ() - zsum, 2);
        }
        ...
        if (pow2sum > 0) {
            double sk = pow3sum / pow(pow2sum, 1.5);
            param.setSkMc(sk);
        }
    }
    */
...
void  CalcParams::calcKrMcs(vector<LasPoint>& points, LandCoverParams& param) {
    ...
    /*
    long long int pow2sum = 0, pow4sum = 0;
    unsigned long pointNum = 0;
    long double zsum = 0;
